...
class Kansas:
    ...
    def __init__(self, points, centers, sigma2="auto", Kernel="gaussian", eps=1e-8):
        ...
        self.r2 = np.float32(spatial.distance.cdist(
            points, centers, 'sqeuclidean'))
        ...
        self.kernel_list = ["gaussian", "MQ", "IMQ"]
    ...
    def get_interpolation_matrix(self):
        ...
        if self.use_optimized:
            ...
            g = sy.lambdify(self.x, self.expr, "numpy")

            G = g(self.r2)  # evaluate radial function on the kernel

        # take off elements to an specified tolerance
        G[np.abs(G) < self.eps] = 0.0
    ...
    def get_cross_derivative_matrix(self, var_index1, var_index2):
        ...
        if self.use_optimized:
            ...
            Dxy = self.get_cross_derivative_matrix_optimized(
    ...
    def kernel_Dxx(self, r2, rx2, sigma2, Kernel_type):
        ...
        if Kernel_type == "gaussian":
            ...
            Dxx = ne.evaluate(
                '(1.0/sqrt(r2 + sigma2))-rx2*((r2 + sigma2)**(-1.5))')
        elif Kernel_type == "IMQ":
    ...
    def kernel_Laplacian(self, r2, sigma2, Kernel_type):
        ...
        if Kernel_type == "gaussian":
            ...
            L = ne.evaluate(
                '(ndim/sqrt(r2 + sigma2))-r2*((r2 + sigma2)**(-1.5))')
