...
from simulai.io import Sampling, MemorySizeEval
...
def construct_IPOD_pipeline(dataset,
                            n_components=10,
                            batch_size=100,
                            train_interval=None,
    ...
    with contextlib.ExitStack() as stack:
        ...
        sampler.prepare_input_structured_data(dataset, data_interval=train_interval, batch_size=batch_size,
...
def pipeline_project_then_reconstruct(dataset,
                                      pipeline=None,
    ...
    if reconstructed_dump_path is not None:
        pipeline_reconstruct(projected_data=projected,
...
def pipeline_reconstruct(projected_data,
                         variables_names,
                         pipeline,
    ...
    pipeline.reconstruct_data(data=projected_data,
...
def dataset_ipod(
    ...
    with h5py.File(data_path, 'r') as fp:
        ...
        n_samples = dataset.shape[0]  # Total number of samples (time-steps) in the dataset
        n_samples_train = int(
            train_interval_fraction * n_samples)  # Rounded value of samples (used for practical purposes)
        ...
        elif isinstance(project_reconstruct_interval, str) and project_reconstruct_interval == 'full_interval':
            project_reconstruct_interval = [0, n_samples]
        elif isinstance(project_reconstruct_interval, str) and project_reconstruct_interval == 'test_interval':
            project_reconstruct_interval = [n_samples_train, n_samples]
        elif isinstance(project_reconstruct_interval, list) and len(project_reconstruct_interval) == 2:
            pass
        else:
            raise RuntimeError(f'Invalid option project_reconstruct_interval={project_reconstruct_interval}')

        pipeline = construct_IPOD_pipeline(dataset,
                                           n_components=n_components,
        ...
        pipeline.save(save_path=save_path, model_name=f'pipeline_{dataset_name}_{data_key}.pkl')

        projected = pipeline_project_then_reconstruct(dataset,
                                                      pipeline=pipeline,
                                                      reconstructed_dump_path=reconstructed_dump_path,
                                                      batch_size=batch_sizer,
        ...
        np.save(os.path.join(save_path, f"time_series_{dataset_name}_{data_key}.npy"), projected)
        ...
        if reconstructed_dump_path is not None and error_norm_dump_path is not None:
            with h5py.File(reconstructed_dump_path, 'r') as fpr:
                ...
                norm_to_test_reference = compute_datasets_to_reference_norm(reconstructed,
                                                                            dataset,
...
def pipeline_projection_error(
    ...
    with h5py.File(data_path, 'r') as fp:
        ...
        with contextlib.ExitStack() as stack:
            if reconstructed_dump_path is None or os.path.isdir(reconstructed_dump_path):
                ...
                if scaled:
                    projected = projected * np.reshape(np.sqrt(pipeline.rom.singular_values[0:projected.shape[1]]), [1, -1])

                if n_sub_components is not None:
                    pass
                elif energy_preserved is not None:
                    cum_explained_variance_ratio = np.cumsum(pipeline.rom.pca.explained_variance_ratio_)
            ...
            with h5py.File(reconstructed_dump_path, 'r') as fr:
                ...
                reconstruction_error = compute_datasets_to_reference_norm(data=reconstructed,
                                                                          reference_data=dataset,
                                                                          reference_data_interval=projected_data_interval,
                                                                          batch_size=batch_sizer)

                if error_filename is None:
                    append_name = os.path.splitext(os.path.basename(reconstructed_dump_path))[0]
