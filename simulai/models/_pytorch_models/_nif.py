...
class WorkflowModule(torch.nn.Module):
    ...
    def forward(
        self, parameters: torch.Tensor = None, input_tensor: torch.Tensor = None
    ):
...
class NIF(NetworkTemplate):
    ...
    def __init__(
        ...
        self.parameter_network = self.to_wrap(
            entity=parameter_network, device=self.device
        )
        ...
        # operation
        self.latent_projection = Linear(
            input_size=self.n_outputs_parameter, output_size=self.n_shape_parameters
        )
        ...
        sample_parameters_tensor = torch.from_numpy(
            np.random.rand(self.n_shape_parameters).astype(ARRAY_DTYPE)
        )
        sample_input_tensor = torch.from_numpy(
            np.random.rand(1_00, self.n_inputs_shape).astype(ARRAY_DTYPE)
        )
    ...
    def eval(
        ...
        output_tensor = self.forward(
            input_shape=shape_data, input_parameter=parameter_data
        )
