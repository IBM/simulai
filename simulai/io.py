...
class MapValid(Reshaper):
    ...
    large_number : int
        the large number used to replace the mask in the data array
    mask : int, np.NaN, np.inf
        the mask used to select the invalid values
    mask_ : int, np.NaN, np.inf
        the original mask passed during initialization
    valid_indices : np.ndarray
        the indices of the valid elements in the data array
    original_dimensions : tuple
        the original dimensions of the data array
    channels_last : bool
        if set to True, move the channel dimension to the last
    default_dtype : str
        default data type of the reshaper
    return_the_same_mask : bool
        if set to True, return the same mask during reshaping
    replace_mask_with_large_number : bool
        if set to True, replace the mask with a large number in the data array
...
class SlidingWindow:
    ...
    def __call__(self, input_data: np.ndarray = None, output_data: np.ndarray = None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        while center + self.skip_size <= data_size:
            ...
            input_batch = input_data[center - self.history_size:center, :]
            output_batch = output_data[center - self.history_size + self.skip_size:center+self.skip_size, :]
