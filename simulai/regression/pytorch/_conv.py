...
class ConvolutionalNetwork(ConvNetworkTemplate):
    ...
    def __init__(self, layers:list=None, activations:list=None, case:str='2d', last_activation:str='identity',
        ...
        self.before_conv_layers, self.conv_layers,\
        self.after_conv_layers, self.weights = self._setup_layers(layers_config=self.layers_config)

        self.list_of_layers = self._merge(before_conv=self.before_conv_layers, conv=self.conv_layers,
                                          act=self.activations, after_conv=self.after_conv_layers)
...
class ResConvolutionalNetwork(ConvNetworkTemplate):
    ...
    def __init__(self, stages:List[list]=None, activations:List[list]=None,
                       case:str='2d', last_activation:str='identity',
        ...
        for (stage, activations) in zip(self.stages, self.activations):
           ...
           before_conv_layers, conv_layers, after_conv_layers, weights = self._setup_layers(layers_config=stage)
           activations_, activations_str = self._setup_activations(activation=activations, n_layers=len(stage))

           layers_sequence = self._merge(before_conv=before_conv_layers, conv=conv_layers, act=activations_,
