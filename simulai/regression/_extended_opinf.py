...
class ExtendedOpInf(OpInf):
    ...
    def fit(self, input_data:np.ndarray=None,
                  ...
                  batch_size:int=None, **kwargs) -> None:
        ...
        if self.fobservables is not None and forcing_data is None:
            raise Exception("There are forcing observables defined, but there is no forcing data provided.")
        elif self.fobservables is None and forcing_data is not None:
            raise Exception("Forcing data was provided, but the forcing observables were not defined.")
        elif self.fobservables is not None and forcing_data is not None:
            forcing_observables = self._generate_observables(data=forcing_data, observables=self.fobservables)
        else:
            forcing_observables = None

        input_observables = self._generate_observables(data=input_data, observables=self.observables)

        super().fit(input_data=input_observables, target_data=target_data,
                    forcing_data=forcing_observables, batch_size=batch_size, **kwargs)
