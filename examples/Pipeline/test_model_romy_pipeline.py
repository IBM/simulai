...
from simulai.normalization import UnitaryNormalization, StandardNormalization

def evaluate_reduced_var_and_time_derivatives(pipeline, test_data, input_data, variables_list):

    test_data_reduced = pipeline.project_data(data=test_data, variables_list=variables_list, mean_component=True)
    input_data_reduced = pipeline.project_data(data=input_data, variables_list=variables_list, mean_component=True)
...
def test_derivatives_error(pipeline, test_data_reduced, input_data_reduced, l2_norm):
    ...
    derivatives_output_test = pipeline.eval(data=test_data_reduced, with_projection=False, with_reconstruction=False)
    derivatives_output = pipeline.eval(data=input_data_reduced, with_projection=False, with_reconstruction=False)

    derivatives_error_test = l2_norm(data=derivatives_output_test,
                                     reference_data=derivatives_test_data_reduced,
                                     relative_norm=True)

    derivatives_error = l2_norm(data=derivatives_output,
...
scipy_config = {'method': 'L-BFGS-B', 'options': {'maxiter': 50000,
                                                  'maxfun': 50000,
                                                  'maxcor': 50,
                                                  'maxls': 50,
...
model_config = {
                ...
                'loss_function': 'mse',
                'optimizers': {'adam': {'n_epochs': 2000, 'config': adam_config},
...
input_data_reduced, \
derivatives_test_data_reduced, \
derivatives_input_data_reduced = evaluate_reduced_var_and_time_derivatives(pipeline, test_data_,
                                                                           input_data, variables_list)
...
derivatives_output_test, derivatives_output = test_derivatives_error(pipeline, test_data_reduced,
                                                                     input_data_reduced, l2_norm)
...
output_reduced = pipeline.predict(post_process_op=RK4, extra_kwargs=extra_kwargs, with_reconstruction=False)
input_vars_list = list(initial_state.dtype.names)
test_data_reduced = pipeline.project_data(test_data_, input_vars_list, mean_component=True)
