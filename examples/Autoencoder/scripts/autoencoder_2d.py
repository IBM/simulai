...
data = BatchwiseSampler(dataset=dataset, input_variables=['u'], target_variables=['u'], channels_first=True)
minimum, maximum = data.minmax(batch_size=1_000, data_interval=[0, n_samples_train])
...
def model():
    ...
    if transpose == False:
        ...
        decoder_layers = [
            ...
            {'in_channels': 16, 'out_channels': n_outputs, 'kernel_size': 2, 'stride': 2},
    ...
    autoencoder = AutoencoderCNN(encoder=encoder, bottleneck_encoder=bottleneck_encoder,
                                 bottleneck_decoder=bottleneck_decoder, decoder=decoder, encoder_activation='tanh')
...
params = {'lambda_1': 0., 'lambda_2': 0., 'weights': [1], 'axis': 1, 'use_mean':False, 'relative':True}

optimizer = Optimizer('adam', params=optimizer_config)

optimizer.fit(op=autoencoder, input_data=data.input_data, target_data=data.target_data,
              n_epochs=n_epochs, loss="wrmse", params=params, batch_size=5_00)

saver = SPFile(compact=False)
saver.write(save_dir=save_path, name='autoencoder_rb', model=autoencoder, template=model)
