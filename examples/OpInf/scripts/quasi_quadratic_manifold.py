...
if norm is not True:
    ...
    data_validation_norm = (
        2 * (data_validation - min_train) / (max_train - min_train) - 1
    )
...
qqm = QQM(
    n_inputs=projected.shape[-1], lambd=1e-5, alpha_0=1e-2, epsilon=1e-3, use_mean=True
)
...
error = 100 * l2_norm(
    data=reconstructed, reference_data=data_test_norm, relative_norm=True
)
