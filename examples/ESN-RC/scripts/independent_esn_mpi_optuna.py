...
class HyperModelPoolESN(HyperTrainTemplate):
    ...
    def __init__(self, trial_config: dict=None, set_type='hard', path_to_model:str =None, other_params: dict=None):
        ...
        required_keys = ['n_field', 'n_forcing', 'model_type', 'sub_model_number_of_inputs',
                         'global_matrix_constructor_str', 'n_numba_workers', 'memory_percent',
                         'parallel', 'id', 'tag']
    ...
    def _set_model(self):
        ...
        self.model = ModelPool(config={'template': 'independent_series',
...
def objective(model=None, initial_state=None, test_data=None, forcings_input_test=None, nt_test=None):

    extrapolation_data = model.predict(initial_state=initial_state,
                                       auxiliary_data=forcings_input_test,
                                       horizon=nt_test)

    l2_norm = L2Norm()

    error = l2_norm(data=extrapolation_data, reference_data=test_data, relative_norm=True)

    return error

def test_modelpool_nonlinear_forcing_numba_MPI_Optuna(n_numba_workers: int=None, n_trials: int=None, parallel: str='mpi'):
...
if __name__ == "__main__":
    ...
    test_modelpool_nonlinear_forcing_numba_MPI_Optuna(n_numba_workers=n_numba_workers,
                                                      n_trials=n_trials,
                                                      parallel=parallel)
