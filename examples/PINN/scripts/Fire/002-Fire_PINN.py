...
dt_init = 1e-02                 # Initial Time Step Size
Last_It_Acepted = 0
...
def adaptative_scale_factors(alpha:float=None):
    ...
    num = abs(state_t)     
    Scalers_New = np.where(num < 0.01, 0.01, num)*2
...
if train == "yes":
    ...
    def model():
        ...
        encoder_u = SLFNN(input_size=1, output_size=width,
                          activation=activations_funct)
        encoder_v = SLFNN(input_size=1, output_size=width,
                          activation=activations_funct)
            ...
            def __init__(self, network=None, encoder_u=None, encoder_v=None, devices="gpu", scale_factors=None):
                ...
                if scale_factors != None:
                    self.scale_factors = torch.from_numpy(
                        scale_factors.astype(ARRAY_DTYPE)).to(self.device)
                else:
    ...
    def model_():
        ...
        class MultiNetwork(NetworkTemplate):
            ...
            def eval(self, input_data: np.ndarray = None) -> np.ndarray:
                ...
                cumulated_time = np.array(
                    [0] + np.cumsum(self.delta_t).tolist())

                input_data = input_data - \
                    cumulated_time[np.array(eval_indices)][:, None]
