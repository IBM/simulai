...
if train == "yes":
    ...
    f_X_C = (
        "D(X_C, t) - ((-Flow_Rate*X_C/Vol)     +         (mumax*S_C/(Ks + S_C))*X_C)"
    )
    f_P_C = (
        "D(P_C, t) - ((-Flow_Rate*P_C/Vol)     +     Ypx*(mumax*S_C/(Ks + S_C))*X_C)"
    )
    f_S_C = (
        "D(S_C, t) - ((Flow_Rate*(Sf-S_C)/Vol) - (1/Yxs)*(mumax*S_C/(Ks + S_C))*X_C)"
    )
    ...
    def Delta_t(i, last_delta_t):
        ...
        if i < Change_Iteration:
            ...
            dt_sug = (
                safety_factor
                * last_delta_t
                * min(max((tol / (2 * LastLoss[-1])) ** (1 / 2), 0.3), 2)
            )
            print("Suggested new Delta t:", dt_sug)
    ...
    while t_acu < t_max:
        ...
        if i == 0:
            estimator = None
            global_estimator = (
                None  # AnnealingWeights(alpha=0.5, init_weight=1e8) #None
            )
        ...
        params = {
            ...
            "residual_weights_estimator": InverseDirichletWeights(
                alpha=0.5, n_residuals=4
            ),  # estimator,
...
# evaluations
    ...
    plt.scatter(
        ...
        ODE_Results_OnlyFewData["Cell Conc."],
        label="ODE - Cell Conc.",
    )
    plt.scatter(
        ODE_Results_OnlyFewData["Time"],
        ODE_Results_OnlyFewData["Product Conc."],
        label="ODE - Product Conc.",
    )
    plt.scatter(
        ODE_Results_OnlyFewData["Time"],
        ODE_Results_OnlyFewData["Substrate Conc."],
    ...
    plt.scatter(
        ODE_Results_OnlyFewData["Time"],
        ODE_Results_OnlyFewData["Volume [liter]"],
