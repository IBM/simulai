...
def model():
    ...
    lorenz_net = ImprovedDeepONet(trunk_network=trunk_net,
...
residual = SymbolicOperator(expressions=[f_x, f_y, f_z], input_vars=input_labels,
                            output_vars=output_labels, function=lorenz_net,
                            inputs_key='input_trunk',
                            constants={"sigma": sigma, "rho": rho,
                                       "bheta": bheta},
                            device='gpu',
                            engine='torch')
...
optimizer.fit(op=lorenz_net, input_data=input_data,
              n_epochs=n_epochs, loss="opirmse", params=params, device='gpu', batch_size=batch_size)

# Saving model
print("Saving model.")
saver = SPFile(compact=False)
saver.write(save_dir=save_path, name='lorenz_deeponet', model=lorenz_net, template=model)
