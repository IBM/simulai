...
t_intv = [0, 1]
s_intv = np.stack([[0, 0, -1], [10, 10, 1]], axis=0)


# In[8]:


f = "D(u, t) + omega_1*pi*sin(omega_1*pi*t)*sin(omega_2*pi*t) - omega_2*pi*cos(omega_1*pi*t)*cos(omega_2*pi*t)"
...
input_labels = ["t", "omega_1", "omega_2"]
output_labels = ["u"]
...
def model():
    ...
    manufactured_net = ImprovedDeepONet(
        ...
        var_dim=n_outputs,
        devices="gpu",
        model_id="manufactured_net",
    )

    return manufactured_net


# In[18]:


manufactured_net = model()
...
saver = SPFile(compact=False)
manufactured_net = saver.read(model_path=save_path)
