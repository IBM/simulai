...
for j in range(n_cases):
    ...
    oscillator_data, _ = oscillator_solver(T, dt, initial_state, extra_params=list_of_cases[j])
...
Input_parameters_train = np.tile(input_parameters_train[:, None, :], (1, n_steps, 1)).reshape(n_steps*n_cases_train, -1)
...
oscillator_net = DeepONet(trunk_network=trunk_net,
...
optimizer.fit(oscillator_net, input_data=input_data, target_data=Field_data_train,
...
for c in range(n_cases_test):
    ...
    approximated_data = oscillator_net.eval(trunk_data=Time_test, branch_data=Input_parameters_test)

    l2_norm = L2Norm()

    error = 100*l2_norm(data=approximated_data, reference_data=Field_data_test, relative_norm=True)
