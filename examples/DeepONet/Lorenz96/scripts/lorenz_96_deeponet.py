...
branch_train_input = np.vstack([np.tile(init_state, (Q, 1)) for init_state in initial_states])
...
branch_test_input = np.vstack([np.tile(init_state, (Q, 1)) for init_state in initial_states])
...
def model():
    ...
    encoder_trunk = SLFNN(input_size=n_inputs_t, output_size=100, activation=activation_et)
