...
def lorenz_solver(
    rho, beta, sigma, T, dt, data_path, initial_state, solver="RK4", **kwargs
) -> object:
    """
    ...
    # Using the SciPy RK4 implementation
        ...
        solution = solve_ivp(
            problem.eval, [0, T], current_state[0], max_step=dt, method="RK45"
        )
    ...
    label_string = "rho_{}_sigma_{}_beta_{}_T_{}_dt_{}/".format(
        rho, sigma, beta_str, T, dt
    )

    all_variables_matrix = np.hstack([variables_matrix, derivatives_matrix])
    n_col = all_variables_matrix.shape[1]
    all_variables_record = np.core.records.fromarrays(
        np.split(all_variables_matrix, n_col, axis=1),
    ...
    if data_path != "no_path":
        ...
        np.save(
            os.path.join(data_path, "{}Lorenz_data.npy".format(label_string)),
            all_variables_record,
        )
...
def lorenz_solver_forcing(
    rho, beta, sigma, T, dt, data_path, initial_state, forcing=None, **kwargs
):
    """
