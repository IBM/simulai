...
class StepwiseExtrapolation:
    ...
    def __init__(self, model: Union[NetworkTemplate, Regression, list, dict]=None,
                       keys: list=None) -> None:
        ...
        if isinstance(model, NetworkTemplate) or isinstance(model, Regression):
            ...
            self.models_instances = OrderedDict({key: item for key, item in zip(keys, model_)})
            ...
            self.models_instances = OrderedDict({key: item for key, item in zip(keys, model)})
            ...
            raise Exception(f"The attribute model must be callable or list(callable), but received {type(model)}")



    def _check_if_callable_and_has_step(self, model: Union[list, dict.values]=None) -> None:

        assert all(
                    [ ((isinstance(item, NetworkTemplate) or
                        isinstance(item, Regression)) and hasattr(model, 'step'))
                                                        for item in model
                                                                          ]
    ...
    def _serial_predict_with_auxiliary(self, initial_state: ndarray=None,
                                             auxiliary_data: ndarray=None,
                                             horizon: int=None) -> ndarray:
    ...
    def predict(self, initial_state: ndarray=None, auxiliary_data: ndarray=None,
                      horizon: int=None, parallel: str=None):
        ...
        return self.extrapolator(initial_state=initial_state,
